    ...
    function drawSabShots() {
        ...
        for (const shot of sabShots) {
            ...
            const targetX = target.id === heroId ? shipX : target.x;
            const targetY = target.id === heroId ? shipY : target.y;

            const posX = startX + (targetX - startX) * progress;
            const posY = startY + (targetY - startY) * progress;
